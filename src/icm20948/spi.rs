...
where
    BUS: spi::SpiDevice<Error = E>,
{
    /// Create and initialize a new IMU driver.
    ///
    /// The SPI bus is given as `bus`
    pub fn new(mut bus: BUS) -> Result<Self, IcmError<E>> {
        let buf = [RegistersBank0::PwrMgmt1.get_addr(WRITE_REG), 0x01];
    ...
    pub fn wai(&mut self) -> Result<u8, IcmError<E>> {
        ...
        self.databuf[1] = 0;
        self.bus.transfer_in_place(&mut self.databuf[0..2])?;
    ...
    pub fn enable_int(&mut self) -> Result<(), IcmError<E>> {
        ...
        self.databuf[1] = 0x01;
        self.bus.write(&mut self.databuf[0..2])?;
    ...
    pub fn read_acc_x(&mut self) -> Result<f32, IcmError<E>> {
        ...
        self.bus.transfer_in_place(&mut self.databuf[0..3])?;
    ...
    pub fn set_acc_sen(&mut self, acc_sen: AccSensitivity) -> Result<(), IcmError<E>> {
        ...
        self.bus.transfer_in_place(&mut self.databuf[2..4])?;
